...
namespace cyberpawn {
    ...
    bool makeMoveIfGeometricallyLegal(ChessPosition & position, const ChessMove & move) {
        ...
        if (pieceToMove.isPawn()) {
            ...
            ChessSquare queensideCastleRookPassthroughSquare = ChessSquare{ 1, castleRank };
            ...
            if (position.mayCastleQueenside() && from == castleStartSquare && to == queensideCastleEndSquare) {
                ...
                else if (position[queensideCastlePassthroughSquare] != PieceCode::noPiece || position[queensideCastleEndSquare] != PieceCode::noPiece || position[queensideCastleRookPassthroughSquare] != PieceCode::noPiece) {
                    return false;
                }
                else {
                    position[to] = pieceToMove;
    ...
    // checks if the piece can move there under normal circumstances (e.g. it's along a bishop's diagonal)
    bool isMoveGeometricallyLegal(ChessPosition const & position, const ChessMove & move) {
        ChessSquare from = move.from;
        ChessSquare to = move.to;
    ...
    */
    ...
    std::vector<ChessMove> allLegalMoves(const ChessPosition & position) {
        std::vector<ChessMove> potentialMoves = collectPotentialMoves(position);
        std::vector<ChessMove> ret;
